	...
	function calcData (callback){
		...
		pool.getConnection(function(err, connection){ // where 'sen_time.ID' = 'sen_calc.ID' //'sen_time',
			...
			if(connectionError){
				...
				connection.query("select sen_calc.SEN_TIME, sen_calc.SEN_CALC , sen_calc.Units from sen_calc where sen_calc.SEN_TIME like '2015-05-22 12:00:0%' limit 1000", function(query_err, result){
					connection.release();
